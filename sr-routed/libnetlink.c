			   ...
			   bool show_rtnl_err, nl_ext_ack_fn_t errfn)
{
	...
	for (i = 0; (unsigned int) i < iovlen; i++) {
		h = iov[i].iov_base;
...
next:
		...
		for (h = (struct nlmsghdr *)buf; status >= (int64_t) sizeof(*h); ) {
			int len = h->nlmsg_len;
			int l = len - sizeof(*h);
			...
			if (h->nlmsg_type == NLMSG_ERROR) {
				...
				if (l < (int64_t) sizeof(struct nlmsgerr)) {
					fprintf(stderr, "ERROR truncated\n");
				} else if (!err->error) {
...
int rta_addattr32(struct rtattr *rta, int maxlen, int type, __u32 data)
{
	...
	if (RTA_ALIGN(rta->rta_len) + len > (unsigned int) maxlen) {
		fprintf(stderr,
		  ...
		  const void *data, int alen)
{
	...
	if (RTA_ALIGN(rta->rta_len) + RTA_ALIGN(len) > (unsigned int) maxlen) {
		fprintf(stderr,
...
int __parse_rtattr_nested_compat(struct rtattr *tb[], int max,
				 ...
				 int len)
{
	if (RTA_PAYLOAD(rta) < (unsigned int) len)
		return -1;
